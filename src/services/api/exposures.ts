...
export const validateCode = async (code: string): Promise<ValidateCodeResponse> => {
  const controlHash = await digestStringAsync(CryptoDigestAlgorithm.SHA512, code.substr(0, 3), {
...
export const uploadExposureKeys = async (uploadToken: string, exposures: any[]): Promise<void> => {
