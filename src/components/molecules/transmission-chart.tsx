...
export const TransmissionChart: FC<TransmissionChartProps> = ({ title, data }) => {
  ...
  return (
    <Card>
      ...
      {data.map(([label, value], idx) => (
        <View key={`ctx-${idx}`} style={styles.chart} accessible accessibilityRole="text">
          ...
          <View style={styles.rightCol}>
            <View key={`progress-${idx}`}>
              <View style={[styles.progress, { width: `${Math.round((value * 100) / maxValue)}%` }]} />
