...
const normalizeString = (s: string): string => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '');

export const LocationDropdown: FC<LocationDropdownProps> = ({ value, onValueChange }) => {
  const { t } = useTranslation();

  const [countyItems, setCountyItems] = useState<CountyOption[]>(withPreferNotToSay(t, countyOptions));
  ...
  const [countyLocalities, setCountyLocalities] = useState<LocalityOption[]>(initialLocalities);
  const [localityItems, setLocalityItems] = useState<LocalityOption[]>(initialLocalities);
  ...
  const onCountySearchChanged = (searchTerm: string) => {
    const items = !searchTerm
      ...
      : countyOptions.filter(c =>
          normalizeString(c.value).toLowerCase().includes(normalizeString(searchTerm).toLowerCase())
        );
  ...
  const onLocalitySearchChanged = (searchTerm: string) => {
    const items = !searchTerm
      ...
      : countyLocalities.filter(l =>
          normalizeString(l.value).toLowerCase().includes(normalizeString(searchTerm).toLowerCase())
        );
