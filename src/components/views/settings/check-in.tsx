...
export const CheckInSettings: React.FC<CheckInSettingsProps> = ({ navigation }) => {
  ...
  const { sex = '', ageRange = '', location = { county: '', locality: '' } } = app.user!;
  ...
  if (!sex || !ageRange || !location.county || !location.locality) {
    return (
      <Basic heading={t('checkInSettings:title')}>
        ...
        <Spacing s={48} />
        <Button type="empty" onPress={() => navigation.navigate('symptoms', { screen: 'symptoms.checker' })}>
  ...
  return (
    <Scrollable toast={successToast} heading={t('checkInSettings:title')} scrollViewRef={scrollViewRef}>
      ...
      <SelectList
        ...
        onItemSelected={value => setProfile({ ...profile, saved: false, sex: value })}
      />
      <Separator />
      <Dropdown
        ...
        onValueChange={value => setProfile({ ...profile, saved: false, ageRange: value })}
      />
      <Separator />
      <LocationDropdown
        value={profile.location}
        onValueChange={value => setProfile({ ...profile, saved: false, location: value })}
      />
      <Spacing s={24} />
      <Button
        disabled={
