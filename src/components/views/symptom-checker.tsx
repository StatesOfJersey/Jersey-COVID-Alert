...
import { Animated, Image, Text, StyleSheet, View, Dimensions } from 'react-native';
...
export const SymptomChecker: FC<SymptomCheckerProps> = ({ route }) => {
  ...
  const [state, setState] = useState({
    question:
      app.user && (!app.user.sex || !app.user.ageRange || !app.user.location.county || !app.user.location.locality)
        ? -1
        : skipQuickCheckIn
        ? 1
        : 0,
    ...
    location: Object.assign({ county: '', locality: '' }, app.user && app.user.location),
    feelingWell: true,
    answers: {
      fever: 0,
      cough: 0,
  ...
  if (!app.checkInConsent) {
    return (
      <Scrollable safeArea={false} backgroundColor="#FAFAFA">
        <Heading accessibilityRefocus accessibilityFocus text={t('checker:title')} />
        <CheckInConsent />
      </Scrollable>
    );
  }
  ...
  const renderIntro = () => (
    <>
      <Card padding={{ v: 12, h: 12 }}>
        ...
        />
        <Separator />
        <LocationDropdown value={state.location} onValueChange={location => setState(s => ({ ...s, location }))} />
      </Card>
      <Spacing s={16} />
  ...
  const headingTranslationKey = state.result ? 'resultTitle' : state.question === -1 ? 'introTitle' : 'title';

  return (
    <Scrollable safeArea={false} backgroundColor="#FAFAFA">
      <Heading accessibilityRefocus accessibilityFocus text={t(`checker:${headingTranslationKey}`)} />
      ...
      {state.question > 0 && state.question < 5 && (
        <Animated.View style={{ transform: [{ translateX: state.slideInX }] }} accessibilityLiveRegion="polite">
          <Card padding={{ v: 12, h: 12 }}>
            <View style={styles.row}>
              <Image accessibilityIgnoresInvertColors style={styles.icon} source={icons[state.question]} />
              <View style={styles.question}>
                <Markdown markdownStyles={markdownStyles}>{t(`checker:question${state.question}`)}</Markdown>
                <Spacing s={8} />
              </View>
            </View>
            <View style={styles.buttonsContainer}>
              <Button
                ...
                type="empty"
                onPress={() => handleAnswer(t(`checker:question${state.question}id`), 0)}
              >
                {t('checker:no')}
              </Button>
              <Button
                ...
                type="empty"
                onPress={() => handleAnswer(t(`checker:question${state.question}id`), 1)}
              >
                {t('checker:yes')}
              </Button>
            </View>
