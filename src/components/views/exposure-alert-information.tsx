...
const TracingImage = require('assets/images/information/womanPhone.png');
...
export const ExposureAlertInformation: React.FC<Props> = ({ route }) => {
  ...
  const handlePermissions = async () => {
    ...
    if (route?.params?.embedded && app.callBackData) {
  ...
  if (!exposure.supported && !exposure.canSupport) {
    return (
      <PinnedBottom heading={t('onboarding:information:title')}>
        ...
        <Spacing s={20} />
        <Text style={text.default}>{t('exposureAlert:noSupport:message')}</Text>
        <Button
          onPress={() => {
            navigation.reset({
              index: 0,
  ...
  const permissionsInfo = (
    <>
      ...
      <View style={permissionsStyles.row}>
        <AppIcons.Bluetooth style={permissionsStyles.icon} width={24} height={24} color={colors.darkGray} />
        <Text style={text.defaultBold}>{t('onboarding:information:bluetooth')}</Text>
      </View>
      {Platform.OS !== 'android' && (
        <>
          <Spacing s={20} />
          ...
          <Spacing s={20} />
          <View style={permissionsStyles.row}>
            <AppIcons.Notification style={permissionsStyles.icon} width={24} height={24} color={colors.darkGray} />
            <Text style={text.defaultBold}>{t('onboarding:information:notifications')}</Text>
          </View>
        </>
      )}
      <Spacing s={20} />
      ...
      <Spacing s={32} />
      <Button onPress={handlePermissions}>{t('onboarding:information:action')}</Button>
      <Spacing s={28} />
      ...
      <Card>
        <SingleRow>
          <View style={cardStyles.text}>
            <Text style={text.defaultBold}>{t(`onboarding:upgrade:${Platform.OS}:title`)}</Text>
            <Spacing s={12} />
            <Text style={text.default}>{t(`onboarding:upgrade:${Platform.OS}:text`)}</Text>
          </View>
          <View>
            <Image
              accessibilityIgnoresInvertColors
        ...
        <Spacing s={16} />
        <Button onPress={checkForUpgradeHandler}>{t(`onboarding:upgrade:${Platform.OS}:action`)}</Button>
      </Card>
      <Spacing s={28} />
      <Link
        align="center"
  ...
  return (
    <Scrollable heading={t('onboarding:information:title')}>
