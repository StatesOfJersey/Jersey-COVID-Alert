...
export const CountyBreakdown = () => {
  ...
  const total = app.data ? app.data.counties.reduce((acc, { cases }) => acc + cases, 0) : 0;
  const max = app.data ? Math.max(...app.data.counties.map(({ cases }) => cases)) : 0;
        ...
        <Card padding={{ v: 12 }}>
          {app.data.counties.map(({ county, cases }, index) => {
            ...
            return (
              <View
                key={`county-${county}`}
                style={[styles.line, index === (app.data && app.data.counties.length - 1) && styles.lastLine]}
              >
                ...
                <View style={styles.right}>
                  <View style={styles.progressWrapper}>
                    <View style={[styles.progress, { width: `${widthPercentage}%` }]} />
                  </View>
