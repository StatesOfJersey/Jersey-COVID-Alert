...
export const ApplicationContext = createContext(initialState as ApplicationContextValue);
...
export const AP = ({ appConfig, user, consent, children }: API) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (state.user) {
        ...
        chartsTabIndex = Number(await AsyncStorage.getItem('cti.chartsTabIndex'));
      }
      ...
      const analyticsConsentItem = await SecureStore.getItemAsync('analyticsConsent');
      const analyticsConsent = analyticsConsentItem === 'true';
  ...
  const setContext = async (data: Partial<State>) => {
    ...
    if (data.callBackData) {
      await SecureStore.setItemAsync('cti.callBack', JSON.stringify(data.callBackData));
    }
    if (data.chartsTabIndex !== undefined) {
      await AsyncStorage.setItem('cti.chartsTabIndex', `${data.chartsTabIndex}`);
    }
  };
  ...
  const checkIn = async (
    ...
    try {
      const lastAllowedDate = startOfDay(subDays(new Date(), appConfig.checkInMaxAge));
  ...
  const verifyCheckerStatusRef = useCallback(verifyCheckerStatus, [state.completedChecker, state.completedCheckerDate]);
